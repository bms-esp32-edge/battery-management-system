 ...
 * - Safe for cross-task/ISR synchronization via critical sections (Standard Layout, Trivially Copyable)
 */
...
struct PyroTriggerKey {
    static constexpr uint32_t ARM_MAGIC             = 0x5A5AA5A5U;  ///< Alternating bit pattern (Hamming distance = 16)
    static constexpr uint32_t FIRE_MAGIC            = 0xC3C33C3CU;  ///< Orthogonal bit pattern (Hamming distance = 16)
    static constexpr uint32_t ARM_WINDOW_TIMEOUT_MS = 50U;          ///< 50ms maximum time-to-fire window after arming
...
enum class BmsState : uint8_t {
    ...
    CHARGING       = 2U,  ///< Contactor closed to charger; positive current flow; charging protections active.
    DISCHARGING    = 3U,  ///< Contactor closed to load; negative current flow; under-voltage monitoring active.
    BALANCING      = 4U,  ///< Passive or active cell balance circuitry engaged in standby or charge maintenance.
    FAULT_DEGRADED = 5U,  ///< Recoverable fault (e.g. cell swelling warning, mild thermal excursion); reduced limits.
    FAULT_CRITICAL = 6U,  ///< Immediate software shutdown; main contactors opened immediately.
    TRIP_DETONATED = 7U   ///< Passive post-event latched state reflecting that the pyro-fuse has been blown.
};
...
enum class CellStatus : uint8_t {
    ...
    SWELLING_WARN = 3U,  ///< Strain gauge / force sensor reports physical expansion beyond baseline.
...
enum class FaultCode : uint16_t {
    ...
    THERMAL_RUNAWAY   = 1U << 7,        ///< 0x0080: Thermal rise velocity exceeds catastrophic threshold (dT/dt > 2.0°C/s).
...
public:
    ...
    constexpr void set(FaultCode code) noexcept {
        mask_ |= static_cast<uint16_t>(code);
    }
    ...
    [[nodiscard]] constexpr uint16_t raw_value() const noexcept {
        return mask_;
    }
...
public:
    ...
    constexpr void set(MetricValidity v) noexcept {
    ...
    [[nodiscard]] constexpr bool is_valid() const noexcept {
        return has(MetricValidity::VALID) && !has(MetricValidity::STALE) && !has(MetricValidity::FAULT_COMM);
    }
...
struct CellMetrics {
    ...
    float temperature_c{0.0f};             ///< Thermistor measurement localized to cell casing (°C), resolution: 0.1°C
    float swelling_force_n{0.0f};          ///< Mechanical force or strain measurement in Newtons (N), resolution: 0.05N
 ...
 */
struct PackMetrics {
    float total_voltage_v{0.0f};           ///< Sum of series cell potentials or pack-level terminal voltage (V)
    float pack_current_a{0.0f};            ///< Instantaneous shunt/Hall-effect sensor reading (A) (+ charge, - discharge)
...
[[nodiscard]] constexpr inline const char* to_string(BmsState state) noexcept {
    switch (state) {
        ...
        case BmsState::FAULT_DEGRADED: return "FAULT_DEGRADED";
        case BmsState::FAULT_CRITICAL: return "FAULT_CRITICAL";
        case BmsState::TRIP_DETONATED: return "TRIP_DETONATED";
        default:                       return "UNKNOWN_BMS_STATE";
    }
}
...
[[nodiscard]] constexpr inline const char* to_string(CellStatus status) noexcept {
    switch (status) {
        case CellStatus::ACTIVE:        return "ACTIVE";
        case CellStatus::BALANCING:     return "BALANCING";
        case CellStatus::BYPASSED:      return "BYPASSED";
        case CellStatus::SWELLING_WARN: return "SWELLING_WARN";
        case CellStatus::FAULTED:       return "FAULTED";
        default:                        return "UNKNOWN_CELL_STATUS";
    }
}
...
[[nodiscard]] constexpr inline const char* to_string(FaultCode fault) noexcept {
    switch (fault) {
        ...
        case FaultCode::OVP:               return "OVP";
        case FaultCode::UVP:               return "UVP";
        case FaultCode::OCP_CHARGE:        return "OCP_CHARGE";
        case FaultCode::OCP_DISCHARGE:     return "OCP_DISCHARGE";
        case FaultCode::OTC:               return "OTC";
        case FaultCode::UTC:               return "UTC";
        case FaultCode::SHORT_CIRCUIT:     return "SHORT_CIRCUIT";
        case FaultCode::THERMAL_RUNAWAY:   return "THERMAL_RUNAWAY";
        case FaultCode::SWELLING_CRITICAL: return "SWELLING_CRITICAL";
...
[[nodiscard]] constexpr inline const char* to_string(MetricValidity validity) noexcept {
    switch (validity) {
        ...
        case MetricValidity::FAULT_COMM:  return "FAULT_COMM";
        case MetricValidity::CALIBRATING: return "CALIBRATING";
        default:                          return "UNKNOWN_VALIDITY";
    }
}
...
static_assert((static_cast<uint16_t>(FaultCode::OCP_CHARGE) & static_cast<uint16_t>(FaultCode::OCP_DISCHARGE)) == 0,
              "FaultCode bits must be strictly non-overlapping");
static_assert((static_cast<uint16_t>(FaultCode::OTC) & static_cast<uint16_t>(FaultCode::UTC)) == 0,
              "FaultCode bits must be strictly non-overlapping");
static_assert((static_cast<uint16_t>(FaultCode::SHORT_CIRCUIT) & static_cast<uint16_t>(FaultCode::THERMAL_RUNAWAY)) == 0,
              "FaultCode bits must be strictly non-overlapping");
static_assert((static_cast<uint16_t>(FaultCode::SWELLING_CRITICAL) & static_cast<uint16_t>(FaultCode::COMM_TIMEOUT)) == 0,
              "FaultCode bits must be strictly non-overlapping");
...
static_assert(std::is_standard_layout_v<PyroTriggerKey>, "PyroTriggerKey must have standard layout");
static_assert(std::is_trivially_copyable_v<PyroTriggerKey>, "PyroTriggerKey must be trivially copyable");
